...
T& Matrix<T>::operator[](const int row_index) const  //element access operator
{
    ...
    return (*this).mData[(*this).getCols() * row_index];
}
...
Matrix<T> Matrix<T>::operator* (const Matrix& second_matrix) //matrix multiplication
{
    ...
    for (int  i = 0; i < m_rowSize; i++)
    {
        for (int j = 0; j < second_matrix.getCols(); j++)
        {
            ...
            {
                m_multiplied.mData[i*m_colSize+j]+=(*this).mData[i*m_colSize+k] * second_matrix.mData[k*second_matrix.m_colSize + j];
            }
...
template<typename T>
std::ostream& operator<< (std::ostream& out, const Matrix<T>& given_matrix)
{
    out << given_matrix.getRows()<<" x "<<given_matrix.getCols()<<" matrix:"<<"\n";
